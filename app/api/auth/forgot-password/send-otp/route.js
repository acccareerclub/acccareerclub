// app/api/auth/forgot-password/send-otp/route.js
...
export async function POST(request) {
  try {
    ...
    const user = await User.findOne({ 
      studentId: collegeId.toUpperCase().trim() 
    });

    if (!user) {
      return NextResponse.json({
        success: false,
        message: 'No account found with this College ID'
      }, { status: 404 });
    }
    ...
    // Send OTP via email using your existing sendEmail function
    const emailSubject = 'Password Reset OTP - ACC Career Club';
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f9f9f9; border-radius: 10px;">
        ...
        <div style="background: white; padding: 30px; border-radius: 0 0 10px 10px;">
          <h2 style="color: #3D444C; margin-top: 0;">Password Reset Request</h2>
          
          <p style="color: #555;">Dear ${user.fullName},</p>
          
          <p style="color: #555;">We received a request to reset your password for your ACC Career Club account.</p>
          
          <div style="background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
            <p style="margin: 0; color: #555; font-size: 14px;">Your OTP for password reset is:</p>
          ...
          <p style="color: #555; font-size: 14px;">If you didn't request a password reset, please ignore this email or contact support immediately.</p>
    ...
    return NextResponse.json({
      ...
      email: user.email // For internal use, not sent to client
    });
