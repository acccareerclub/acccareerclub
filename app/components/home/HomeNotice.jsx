...
const HomeNotice = () => {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        setLoading(true);
        // Switched limit to 5, which looks better for a list view
        const response = await fetch("/api/users/notice/get-notice?page=1&limit=5&sortBy=newest");
    ...
    return (
      <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 md:p-8 w-full max-w-4xl mx-auto">
        ...
        <div className="space-y-3">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="animate-pulse flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Stay updated with the latest announcements</p>
          </div>
        </div>
        ...
        {notices.map((notice) => (
          <Link
            ...
            className={`group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-gray-100 bg-white hover:bg-gray-50 hover:border-[#D3A16D]/40 hover:shadow-md transition-all duration-200 relative overflow-hidden ${
              notice.priority === "urgent" ? "border-l-4 border-l-red-500 pl-3 sm:pl-4" : ""
            }`}
          >
            ...
            <div className="flex items-center shrink-0">
              <span className={`text-[11px] px-3 py-1 rounded-full flex items-center gap-1.5 uppercase tracking-wider font-semibold ${getPriorityColor(notice.priority)}`}>
                {notice.priority === "urgent" && <FaExclamationTriangle className="animate-pulse" />}
